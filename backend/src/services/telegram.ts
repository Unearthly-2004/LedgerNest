...
adminBot.start(async (ctx: any) => {
  ...
  if (!business) {
    ...
    const message =
      '🏪 *Welcome to LedgerNest Admin Bot!*\n\n' +
      'You are not linked to a business yet.\n' +
      'Open your LedgerNest dashboard and connect Telegram in Settings.';
...
adminBot.command('link', async (ctx: any) => {
  ...
  if (!businessId) {
    await ctx.reply('❌ Missing link code. Open the LedgerNest dashboard and copy the admin start command.');
    return;
  }
...
adminBot.on(message('text'), async (ctx: any) => {
  ...
  if (!business) {
    await ctx.reply('❌ Not linked to a business. Open LedgerNest dashboard first.');
    return;
  }
...
clientBot.start(async (ctx: any) => {
  ...
  if (!client) {
    await ctx.reply('Welcome to LedgerNest! Ask your shopkeeper to share your account QR code.');
    return;
  }
...
clientBot.action('PAY_USDC', async (ctx: any) => {
  ...
  if (isLocalOnlyUrl(paymentUrl)) {
    await ctx.editMessageText(
      ...
      `Open this link on the same machine running LedgerNest:\n${paymentUrl}`
    );
    await ctx.answerCbQuery();
    return;
  }
...
async function launchTelegramPolling() {
  await Promise.all([
    adminBot.telegram.deleteWebhook(),
    clientBot.telegram.deleteWebhook(),
    adminBot.telegram.getMe(),
    clientBot.telegram.getMe()
  ]);

  // In polling mode Telegraf keeps the launch promise alive while the bot runs.
  // Kick off polling in the background after auth succeeds, then mark transport ready.
  void adminBot.launch().catch((error) => {
    markTelegramTransportError(error);
    console.error('Admin bot polling stopped:', error);
  });
  void clientBot.launch().catch((error) => {
    markTelegramTransportError(error);
    console.error('Client bot polling stopped:', error);
  });
