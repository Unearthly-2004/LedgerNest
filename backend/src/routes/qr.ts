...
router.get('/client/:clientId', async (req: Request, res: Response) => {
  try {
    ...
    const botUsername = process.env.TELEGRAM_CLIENT_BOT_USERNAME || 'LedgerNestClientBot';
    ...
    const qrDataUrl = await QRCode.toDataURL(telegramLink, {
      width: 400,
      margin: 2,
      color: { dark: '#00D084', light: '#0F1117' } // LedgerNest colors
    });
...
router.get('/admin/link', async (req: Request, res: Response) => {
  try {
    ...
    const botUsername = process.env.TELEGRAM_ADMIN_BOT_USERNAME || 'LedgerNestAdminBot';
    let telegramSetupError: string | null = null;
